...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> _onLogin(
    ...
    try {
      ...
      await UserSessionService.storeUserData(resp.data.user);

      // Schedule class reminders for students
      if (resp.data.user.role.toLowerCase() == 'student') {
        await NotificationManager().scheduleTodaysClassReminders();
        // Also check for new assignments and notify locally
        await NotificationManager().checkForNewAssignmentsForStudent();
      }
  ...
  Future<void> _onLogout(
    ...
    try {
      ...
      // Clear user session data and notifications
      await UserSessionService.clearUserData();
      await NotificationManager().clearAllClassNotifications();
  ...
  Future<void> _onRegister(
    ...
    try {
      ...
      emit(state.copyWith(status: AuthStatus.unauthenticated, user: resp.user));
    } catch (e) {
      emit(
        state.copyWith(
          status: AuthStatus.failure,
